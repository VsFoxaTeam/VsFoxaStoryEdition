...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	#if !html5
	var zoomList:Array<Float> = [0.5, 0.75, 1, 1.5, 2, 3, 4, 6, 8, 12, 16, 24];
	#else // The grid gets all black when over 1/12 snap
	var zoomList:Array<Float> = [0.5, 0.75, 1, 1.5, 2, 3, 4, 6, 8, 12,];
	#end
	...
	function addSongUI():Void
	{
			...
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(CoolUtil.difficulties, true), function(difficulty:String)
		{
			if (PlayState.storyDifficulty != Std.parseInt(difficulty))
			{
				...
				{
					trace("File " + _song.song.toLowerCase() + CoolUtil.getDifficultyFilePath() + " is not found. Did you forget something?");
				}
			}
		});
